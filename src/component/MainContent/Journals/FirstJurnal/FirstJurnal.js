...
const FirstJurnal = () =>{
    ...
    useEffect(() => {
        const handleScroll = () => {
          const component = document.getElementById('imgContainer');
          if (component) {
          ...
          const componentTitle = document.getElementById('jurnalFirstTitle');
          if (componentTitle) {
            const rect = componentTitle.getBoundingClientRect();
            const isVisible = rect.top < window.innerHeight;
            if(isVisible && !animate){
                setAnimate(isVisible);
            }
          ...
          const componentDesc = document.getElementById('jurnalFirstDesc');
          if (componentDesc) {
            const rect = componentDesc.getBoundingClientRect();
            const isVisible = rect.top < window.innerHeight;
            if(isVisible && !animate){
                setAnimate(isVisible);
            }
          ...
          const componentBtn = document.getElementById('jurnalFirstBtn');
          if (componentBtn) {
            const rect = componentBtn.getBoundingClientRect();
            const isVisible = rect.top < window.innerHeight;
            if(isVisible && !animate){
                setAnimate(isVisible);
            }
    ...
    return(
        <div className="firstContainer" >
            <div className="firstContainer__contentText" >
                <span id="jurnalFirstTitle" className={`firstContainer__contentText__title ${animate && 'animateTitle'}`} >{title}</span>
                <p id="jurnalFirstDesc" className={`${animate && 'animateDesc'}`} >{desc}</p>
                <button id="jurnalFirstBtn" className={`${animate && 'animateBtn'}`} >{btnText}</button>
            </div>
            <div id="imgContainer" className={` ${animate && 'activeJournalAnimate'} firstContainer__img`} >
